...
#filename <- args[1]
#outname <-arg[2]
filename="salmon83.NumReads.10filter"
#metafile="sample.map.xls"
...
#A2$Treat <- factor(A2$Treat, levels = order_levels, ordered = TRUE)
...
# Filter the first point in each treatment group for labeling
# first_points <- pcaData %>%
#   group_by(Treat) %>%
#   slice(1) %>%
#   ungroup()
first_points <- as_tibble(as.data.frame(pcaData)) %>%
  group_by(Treat) %>%
  slice(1) %>%
  ungroup()
...
custom_colors <- c("NCColgCP" = "black", 
                   #"GER" = "#F5C710","KR1" = "#ff7f00", 
                   "Formula1" = "cyan", 
                   #"TNF" = "#a65628",
                   "Formula2" = "green",
                    #"SLS" = "#66a61e", "SBA3" = "#2297E6","KR16" = "#61D04F","KR3" = "#a6761d",
                    "Formula4" = "hotpink",   
                    #"KR5" = "red", "KR6" = "magenta","KR7" = "blue",  
                    "Formula5" = "purple",
                   "NoTreat"="yellow"
                   #, "OOO" = "slateblue2", "SOFT"="palegreen1", "SQUA" = "#DF536B","SSS" = "#999999",
                   #"SUN1"= "pink", "SUN2" = "lightblue"
)
...
# doc <- doc %>%
#   add_slide(layout = "Title and Content", master = "Office Theme") %>%
#   ph_with(value = paste0("session ", filename),
#           location = ph_location_type(type = "title")) %>%
#   # Add the caption text under the plot
#   ph_with(fpar(ftext(text, prop = fp_text(font.size = 10))), 
#           location = ph_location(left = 0.5, top = 6.3, width = 5.5, height = 0.5))
